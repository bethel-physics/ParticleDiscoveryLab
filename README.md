...
Students can start directly from `MuonAnalysis_student.m`, which contains the instruction of how to set their file path and load the workspace. 
Your reference is `MuonAnalysis_solutions.m`. This is a fully-coded solutions guide that you can run in MATLAB to get a set of solutions plots. 
...
**Software setup**: Python should be installed on whatever computers students will use for the exercise. My experience is using [Cygwin](https://www.cygwin.com/) on Windows, which provides a unix-based terminal in which python can be run. There are many other methods -- if your students' computational courses use Python via a certain program, I recommend following that protocol. 
...
**Instructors**: download this Github code! Post the student blank (`PythonAnalysis/MuonAnalysis_student.py`), the pickle file (`PythonAnalysis/DoubleMuParked_100k.pkl`), and `pollsf.py` on your LMS. The `pollsf.py` function file is adapted directly from Alejandro Garcia's [*Numerical Methods for Physics*](https://github.com/AlejGarcia/NM4P/tree/master/MatlabRevised) repository.
...
### To start from the 100k event workspace
Students can start directly from `PythonAnalysis/MuonAnalysis_student.py`, which is already set to import packages and load the pickle file. 
Your reference is `PythonAnalysis/MuonAnalysis_solutions.py`. This is a fully-coded solutions guide that you can run (`python -u PythonAnalysis/MuonAnalysis_solutions.py`) to get a set of solutions plots. 
You will want to **provide students a mass window** in which to search. Tested, working suggestions include:
 * J/psi meson: 2.8 GeV -- 3.5 GeV
 * Upsilon mesons: 8 -- 12 GeV (there are 3 peaks here, so I give this window to the strongest group)
 * Z boson: 60 -- 120 GeV (or for a more challenging fit, assign 30 -- 150)
 ...
 * Execute `python -u PythonAnalysis/NanoAOD2Arrays.py` and provide students with the new pickle file.  
